...
- [[#git][Git]]
  - [[#magit][magit]]
  - [[#magit-log-combined-margin-author--stats][magit log combined margin (author + stats)]]
...
#+RESULTS:
| #[(frame) ((with-selected-frame frame (efs/set-font-faces))) nil] | highlight-indent-guides--auto-set-faces-with-frame | persp-init-frame | evil-init-esc | #[(frame) ((let ((old-frame (selected-frame)) (old-buffer (current-buffer))) (unwind-protect (progn (select-frame frame 'norecord) (efs/set-font-faces)) (if (frame-live-p old-frame) (progn (select-frame old-frame 'norecord))) (if (buffer-live-p old-buffer) (progn (set-buffer old-buffer)))))) (t)] | pgtk-dnd-init-frame |
...
:END:
- https://github.com/radian-software/straight.el/issues/1189
...
** magit log combined margin (author + stats)
- Custom override to show both author/date AND insertion/deletion stats in the magit-log margin
- Note: This runs =git show --shortstat= per commit, so it can be slow on large logs

#+begin_src emacs-lisp
  (with-eval-after-load 'magit-log

    ;; Width function that accounts for both author/date and shortstat columns
    (defun my/magit-log-margin-width (style details details-width)
      "Calculate margin width for combined author+stats display.
  Adds 16 extra columns for the shortstat (insertions/deletions) info."
      (+ (if details (1+ details-width) 0)
         (if (stringp style)
             (length (format-time-string style))
           (+ 2 1
              (if (eq style 'age-abbreviated) 1
                (+ 1 (apply #'max (mapcar (##max (length (nth 1 %))
                                                  (length (nth 2 %)))
                                          magit--age-spec))))))
         1    ; separator between date and stats
         16)) ; shortstat width ("%5s %5s%4s" = ~16 chars)

    ;; Override the margin format function to show both author/date AND shortstat
    (defun magit-log-format-margin (rev author date)
      "Show both author/date AND insertion/deletion stats in the margin."
  ...
  \\(?:, \\([0-9]+\\) deletions?(-)\\)?\\'" line)
            ...
            (magit-make-margin-overlay
             (concat author-date-str " " stat-str))))))

    ;; Use custom width function in magit-log-margin
    (setq magit-log-margin '(t age my/magit-log-margin-width t 18))
    )
#+end_src

#+RESULTS:
| t | age | my/magit-log-margin-width | t | 18 |
...
#+begin_src emacs-lisp
  (use-package flymake-jsts
    ...
    ;; Configure eglot for all JavaScript/TypeScript modes
...
- new changes based on -> https://github.com/gregnewman/gmacs [python-lsp]
#+begin_src emacs-lisp
  ...
  (use-package org-roam
    ...
    (setq org-roam-capture-templates
          ...
          ("p" "project journal" plain (file "~/Dropbox/org-notes/templates/plainTemplate.org")
           ...
           :unnarrowed t)
          ("s" "writing" plain (file "~/Dropbox/org-notes/templates/plainTemplate.org")
           :if-new (file+head "writings/%<%Y%m%d%H%M%S>-${slug}.org" ":PROPERTIES:\n:SSOR: ignore \n:END:\n#+title: ${title} \n#+date: %(format-time-string \"%Y-%m-%d %H:%M\") \n")
           :unnarrowed t)
          ("w" "work" plain (file "~/Dropbox/org-notes/templates/workTemplate.org")
           :if-new (file+head "work/%<%Y%m%d%H%M%S>-${slug}.org" ":PROPERTIES:\n:SSOR: ignore \n:END:\n#+title: ${title} \n#+date: %(format-time-string \"%Y-%m-%d %H:%M\") \n")
           :unnarrowed t)
...
(use-package markdown-mode
  :straight t  ; Ensures use with straight.el
  :mode (("\\.\\(?:md\\|markdown\\|mkd\\|mdown\\|mkdn\\|mdwn\\)\\'" . markdown-mode)
         ("README\\.md\\'" . gfm-mode))  ; File associations
  :init
  (setq markdown-command "/usr/bin/pandoc")  ; Set the Markdown command
  :bind (:map markdown-mode-map
         ("C-c C-e" . markdown-do)))  ; Keybinding in markdown-mode
#+end_src
...
#+begin_src emacs-lisp
    (use-package aider
      ...
      ;; Or chatgpt model
      (setq aider-args '("--model" "o4-mini" "--no-auto-accept-architect"))
...
#+begin_src emacs-lisp
   (defun search-notes(SEARCH_TERM)
     "Search notes on a the notes directory using deadgrep test"
...
** org-store-link
#+begin_src emacs-lisp
(define-key global-map (kbd "C-c C-o") 'org-store-link)
#+end_src
